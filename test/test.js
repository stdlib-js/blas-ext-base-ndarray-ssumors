...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof ssumors, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( ssumors.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function computes the sum of a one-dimensional ndarray using ordinary recursive summation.', function test( t ) {
	var x;
	var v;

	x = new Float32Array( [ 1.0, -2.0, -4.0, 5.0, 0.0, 3.0 ] );
	v = ssumors( [ vector( x, 6, 1, 0 ) ] );
	t.strictEqual( v, 3.0, 'returns expected value' );

	x = new Float32Array( [ -4.0, -5.0 ] );
	v = ssumors( [ vector( x, 2, 1, 0 ) ] );
	t.strictEqual( v, -9.0, 'returns expected value' );

	x = new Float32Array( [ -0.0, 0.0, -0.0 ] );
	v = ssumors( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( isPositiveZerof( v ), true, 'returns expected value' );

	x = new Float32Array( [ NaN ] );
	v = ssumors( [ vector( x, 1, 1, 0 ) ] );
